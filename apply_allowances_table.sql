-- Apply expatriate_pay_run_item_allowances table migration
...
-- Foreign key constraint (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'expatriate_pay_run_item_allowances_item_id_fkey'
  ) THEN
...
-- RLS Policies (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'expatriate_pay_run_item_allowances' 
    AND policyname = 'Allow all access to expatriate_pay_run_item_allowances'
  ) THEN
...
-- Mark migration as applied (optional, for tracking)
INSERT INTO supabase_migrations.schema_migrations(version, name, statements)
VALUES ('20250131000001', 'expatriate_allowances_table', ARRAY[]::text[])
