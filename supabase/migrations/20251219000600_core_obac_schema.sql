...
CREATE POLICY "rbac_assignments_select" ON public.rbac_assignments FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.has_permission('admin.manage_users', 'ORGANIZATION', scope_id));
CREATE POLICY "rbac_grants_select" ON public.rbac_grants FOR SELECT TO authenticated USING (auth.uid() = user_id OR public.has_permission('admin.manage_users', 'ORGANIZATION', scope_id));
