-- Add requester-based criteria support to submit_payrun_for_approval RPC.
-- New criteria fields: creator_user, creator_designation, creator_department, creator_role,
--                      submitter_user, submitter_designation, submitter_department, submitter_role
-- Also fixes a pre-existing variable-shadowing bug in the criteria evaluation loop where both the
-- outer workflow loop and inner criteria loop used the same v_step record variable, causing
-- v_best_workflow_id to always be NULL after the inner loop overwrote v_step.
...
BEGIN
    ...
    -- Uses v_wf_candidate for the outer loop to avoid variable shadowing with inner v_step loop.
    v_best_workflow_id := NULL;
    FOR v_wf_candidate IN
    ...
    LOOP
        ...
        -- Check each criterion for this workflow (all must match — AND logic)
        FOR v_step IN
            SELECT * FROM public.approval_workflow_criteria
            WHERE workflow_id = v_wf_candidate.wf_id ORDER BY sequence_number
        LOOP
            -- Pay run properties
            ...
            -- Creator criteria (pay_runs.created_by)
            ELSIF v_step.field = 'creator_user' THEN
                IF v_payrun.created_by IS NULL OR NOT (
                    v_payrun.created_by::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            ELSIF v_step.field = 'creator_designation' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.employees e
                    WHERE e.user_id = v_payrun.created_by
                      AND e.designation_id::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            ELSIF v_step.field = 'creator_department' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.employees e
                    WHERE e.user_id = v_payrun.created_by
                      AND e.company_unit_id::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            ELSIF v_step.field = 'creator_role' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.rbac_assignments ra
                    WHERE ra.user_id = v_payrun.created_by
                      AND ra.role_code = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            -- Submitter criteria (pay_runs.approval_submitted_by)
            ELSIF v_step.field = 'submitter_user' THEN
                IF v_payrun.approval_submitted_by IS NULL OR NOT (
                    v_payrun.approval_submitted_by::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            ELSIF v_step.field = 'submitter_designation' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.employees e
                    WHERE e.user_id = v_payrun.approval_submitted_by
                      AND e.designation_id::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;

            ELSIF v_step.field = 'submitter_department' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.employees e
                    WHERE e.user_id = v_payrun.approval_submitted_by
                      AND e.company_unit_id::text = ANY(SELECT jsonb_array_elements_text(v_step.value))
            ...
            ELSIF v_step.field = 'submitter_role' THEN
                IF NOT EXISTS (
                    SELECT 1 FROM public.rbac_assignments ra
                    WHERE ra.user_id = v_payrun.approval_submitted_by
                      AND ra.role_code = ANY(SELECT jsonb_array_elements_text(v_step.value))
                ) THEN v_criteria_match := false; EXIT; END IF;
        ...
        IF v_criteria_match THEN
            v_best_workflow_id := v_wf_candidate.wf_id;
            EXIT;
        END IF;
    END LOOP;
