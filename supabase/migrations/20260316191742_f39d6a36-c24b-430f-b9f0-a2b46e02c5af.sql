...
ALTER TABLE public.company_units ADD COLUMN IF NOT EXISTS head_user_id uuid;
...
ALTER TABLE public.approval_workflow_steps ADD COLUMN IF NOT EXISTS approver_designation_id uuid;
ALTER TABLE public.approval_workflow_steps ADD COLUMN IF NOT EXISTS approver_department_id uuid;
ALTER TABLE public.approval_workflow_steps ADD COLUMN IF NOT EXISTS approver_group_id uuid;

-- Phase 4: Criteria
CREATE TABLE IF NOT EXISTS public.approval_workflow_criteria (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Phase 5: Follow-ups
CREATE TABLE IF NOT EXISTS public.approval_workflow_followups (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    workflow_id uuid NOT NULL REFERENCES public.approval_workflows(id) ON DELETE CASCADE,
    is_enabled boolean NOT NULL DEFAULT false,
    followup_type text NOT NULL DEFAULT 'one_time',
...
-- Phase 6: Messages
CREATE TABLE IF NOT EXISTS public.approval_workflow_messages (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    workflow_id uuid NOT NULL REFERENCES public.approval_workflows(id) ON DELETE CASCADE,
    event_type text NOT NULL,
    from_type text NOT NULL DEFAULT 'system',
    to_type text NOT NULL DEFAULT 'current_approver',
...
ALTER TABLE public.approval_workflow_criteria ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.approval_workflow_followups ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.approval_workflow_messages ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can manage approval_groups" ON public.approval_groups
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated users can manage approval_group_members" ON public.approval_group_members
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated users can manage approval_workflow_criteria" ON public.approval_workflow_criteria
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated users can manage approval_workflow_followups" ON public.approval_workflow_followups
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated users can manage approval_workflow_messages" ON public.approval_workflow_messages
    FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Updated at triggers
CREATE TRIGGER set_approval_groups_updated_at BEFORE UPDATE ON public.approval_groups
    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at_now();

CREATE TRIGGER set_approval_workflow_followups_updated_at BEFORE UPDATE ON public.approval_workflow_followups
    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at_now();

CREATE TRIGGER set_approval_workflow_messages_updated_at BEFORE UPDATE ON public.approval_workflow_messages
    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at_now();

-- Update submit_payrun_for_approval RPC to support new approver types
...
BEGIN
    ...
    -- Phase 4: Criteria-based workflow selection
    ...
    LOOP
        ...
        -- Check each criterion for this workflow
        FOR v_step IN 
        ...
        LOOP
            ...
            ELSIF v_step.field = 'payrun_type' THEN
                IF NOT (COALESCE(v_payrun.type, '') = ANY(SELECT jsonb_array_elements_text(v_step.value))) THEN
                    v_criteria_match := false; EXIT;
                END IF;
            END IF;
        END LOOP;
    ...
    -- Fallback: category-based config
    ...
    LOOP
        ...
        IF v_resolved_user_id IS NULL THEN
            IF v_step.approver_type = 'reporting_to' THEN
                -- Get the reporting manager of the first employee in the pay run
            ...
            ELSIF v_step.approver_role IS NOT NULL THEN
                -- Existing role resolution via OBAC
