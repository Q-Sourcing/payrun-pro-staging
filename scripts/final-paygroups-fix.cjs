...
/**
 * 🔧 Final PayGroups Schema Fix
 * 
 * Fixes the pay_groups table schema mismatch
 */
...
class PayGroupsFixer {
  ...
  async insertPayGroupsData(data) {
    ...
    const transformedData = data.map(row => {
      // Create a clean row with only the columns that exist in staging
      const cleanRow = {
        id: row.id,
        name: row.name,
      ...
      // Add any other columns that exist in both schemas
      if (row.pay_frequency) cleanRow.pay_frequency = row.pay_frequency;
      if (row.pay_type) cleanRow.pay_type = row.pay_type;
      if (row.status) cleanRow.status = row.status;

      return cleanRow;
    });

    console.log(`${colors.cyan}  🔧 Transformed ${transformedData.length} pay_groups rows for staging schema${colors.reset}`);
  ...
  async fixPayGroups() {
    try {
      console.log(`${colors.blue}🔄 Fixing pay_groups schema mismatch...${colors.reset}`);
  ...
  async runFinalFix() {
    try {
      ...
      console.log('🔧 Final PayGroups Schema Fix');
      console.log('=============================');
...
// Main execution
async function main() {
  const fixer = new PayGroupsFixer();
