  ...
  loadEnvironmentVariables() {
    ...
    if (existsSync('.env')) {
      try {
        ...
        console.warn(`${colors.yellow}⚠️ Warning: Could not read .env${colors.reset}`);
      }
    }

    // Also check process.env for any loaded variables
    Object.keys(process.env).forEach(key => {
      if (key.startsWith('VITE_') || key.startsWith('NEXT_PUBLIC_') || key === 'NODE_ENV') {
        if (!envVars[key]) { // Only use process.env if not already set in .env
