...
function resolveTargetEnv() {
  ...
  // Default behavior used by deploy/CI:
  // - NODE_ENV=production => production config
  // - anything else => staging config (since .env.development is not present)
...
function loadEnvForTarget(targetEnv) {
  // Prefer the env file generated by env-manager.js (so values injected at runtime are included).
...
function validateEnvironment() {
  ...
  const varsToValidate = [...requiredVars, ...optionalVars];
  varsToValidate.forEach(varName => {
  ...
  const validationLog = {
    ...
    supabaseProject,
    validationResults,
    allValid: requiredVars.every(varName => validationResults[varName]?.hasRealValue)
  };
