...
import { MultiTenantPayrollService, MultiTenantPayRunItem } from "@/lib/services/multi-tenant-payroll";
import { motion, AnimatePresence } from "framer-motion";
import { supabase } from "@/integrations/supabase/client";

interface MultiTenantExpatriatePayRunDetailsProps {
  payRunId: string;
  payGroup: {
    id: string;
    name: string;
...
interface ExpatriatePayRunItem extends MultiTenantPayRunItem {
  employee?: {
    id: string;
    first_name: string;
    middle_name?: string;
...
export const MultiTenantExpatriatePayRunDetails: React.FC<MultiTenantExpatriatePayRunDetailsProps> = ({
  payRunId,
  payGroup,
  payRunData,
  employees,
  ...
  const exchangeRate = payRunData?.exchange_rate ?? 1;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`multi-tenant-expatriate-payrun-details-${payRunId}`)
      .on("postgres_changes", 
        { 
          event: "*", 
          schema: "public", 
          table: "pay_run_items",
          filter: `master_payroll_id=eq.${payRunId}`
        }, 
        () => {
          console.log("🔄 Real-time update: pay run items changed for this pay run");
          loadExpatriatePayRunItems();
        }
      )
      .subscribe();
  ...
  const loadExpatriatePayRunItems = async () => {
    try {
      setIsLoading(true);
      const items = await MultiTenantPayrollService.getPayRunItems(payRunId);
      setPayRunItems(items as ExpatriatePayRunItem[]);
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to load pay run items',
        variant: 'destructive'
      });
    } finally {
      setIsLoading(false);
  ...
  const calculateExpatriatePay = async (item: ExpatriatePayRunItem, edits: Partial<ExpatriatePayRunItem> = {}) => {
    ...
    try {
      ...
      const result = await MultiTenantPayrollService.calculateExpatriatePay({
        employee_id: updatedItem.employee_id,
        daily_rate: updatedItem.foreign_base_rate || 0,
        days_worked: updatedItem.days_worked || 0,
        allowances: updatedItem.foreign_allowances || 0,
        currency: updatedItem.foreign_currency || payGroup.currency,
        exchange_rate_to_local: updatedItem.exchange_rate || exchangeRate,
        tax_country: updatedItem.tax_country || 'UG'
      });

      const calculatedItem: ExpatriatePayRunItem = {
        ...updatedItem,
        foreign_gross: result.gross_foreign,
        foreign_net: result.net_foreign,
  ...
  const handleFieldChange = async (employeeId: string, field: keyof ExpatriatePayRunItem, value: any) => {
    ...
    if (['foreign_base_rate', 'days_worked', 'foreign_allowances', 'exchange_rate'].includes(field)) {
      const updatedItem = { ...currentItem, ...edits };
      
      if (updatedItem.foreign_base_rate && updatedItem.foreign_base_rate > 0 && 
          updatedItem.days_worked && updatedItem.days_worked > 0 && 
          updatedItem.exchange_rate && updatedItem.exchange_rate > 0) {
        const calculatedItem = await calculateExpatriatePay(updatedItem);
        setEditingItems(prev => ({ 
          ...prev, 
          [employeeId]: { 
            ...edits, 
            foreign_gross: calculatedItem.foreign_gross,
            foreign_net: calculatedItem.foreign_net,
  ...
  const saveExpatriatePayRunItem = async (employeeId: string) => {
    try {
      ...
      await MultiTenantPayrollService.upsertPayRunItem(calculatedItem);
      await MultiTenantPayrollService.updatePayRunTotals(payRunId);
      ...
      toast({
        title: 'Success',
        description: 'Pay run item updated successfully'
      });
      ...
      toast({
        title: 'Error',
        description: 'Failed to save pay run item',
        variant: 'destructive'
      });
    }
  };
  ...
  const addEmployeeToPayRun = async (employee: any) => {
    try {
      ...
      toast({
        title: 'Success',
        description: 'Employee added to pay run'
      });
  ...
  const summaryTotals = useMemo(() => {
    const items = payRunItems.map(item => {
      ...
      const currency = item.foreign_currency || payGroup.currency;
      const rate = edits.foreign_base_rate ?? item.foreign_base_rate ?? 0;
      const days = edits.days_worked ?? item.days_worked ?? 0;
      const allowances = edits.foreign_allowances ?? item.foreign_allowances ?? 0;
      const exchange = item.exchange_rate || exchangeRate || 1;
  ...
  }, [payRunItems, editingItems, payGroup, exchangeRate]);
      ...
      <Card className="border-l-4 border-l-primary shadow-sm">
        <CardHeader className="pb-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">{payGroup.name}</CardTitle>
            <Badge variant="secondary" className="text-xs">
              {payGroup.currency} → Local
            </Badge>
          </div>
          <CardDescription className="text-sm">
            Multi-Tenant Expatriate PayRun: Daily rate payroll with dual currency calculations
          </CardDescription>
        </CardHeader>
        <CardContent className="pb-3">
          <div className="grid grid-cols-4 gap-3 text-xs">
            <div>
              <Label className="text-muted-foreground text-xs">Pay Frequency</Label>
              <p className="font-medium text-sm">{payGroup.pay_frequency}</p>
            </div>
            <div>
              <Label className="text-muted-foreground text-xs">Currency</Label>
              <p className="font-medium text-sm">{payGroup.currency}</p>
            </div>
            <div>
              <Label className="text-muted-foreground text-xs">Exchange Rate</Label>
              <p className="font-medium text-sm">{exchangeRate.toFixed(4)}</p>
            </div>
            <div>
          ...
          <CardContent className="p-2">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-sm mt-1">
                {MultiTenantPayrollService.formatCurrency(summaryTotals.totalGrossForeign, payGroup.currency)}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border rounded-lg shadow-sm">
          <CardContent className="p-2">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-sm mt-1">
                {MultiTenantPayrollService.formatCurrency(summaryTotals.totalNetForeign, payGroup.currency)}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border rounded-lg shadow-sm">
          <CardContent className="p-2">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-sm mt-1">
                {MultiTenantPayrollService.formatCurrency(summaryTotals.totalNetLocal, 'UGX')}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border rounded-lg shadow-sm">
          <CardContent className="p-2">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-sm mt-1">
                {MultiTenantPayrollService.formatCurrency(summaryTotals.totalGrossLocal, 'UGX')}
              </span>
            </div>
          </CardContent>
        </Card>
            ...
            <Table className="text-sm [&_td]:py-2 [&_th]:py-2">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Daily Rate ({payGroup.currency})</TableHead>
                  <TableHead>Days Worked</TableHead>
                  <TableHead>Base Earnings ({payGroup.currency})</TableHead>
                  <TableHead>Allowances ({payGroup.currency})</TableHead>
                  <TableHead>Net ({payGroup.currency})</TableHead>
                  <TableHead>Net (UGX)</TableHead>
                  <TableHead>Gross (UGX)</TableHead>
                  <TableHead>Gross ({payGroup.currency})</TableHead>
              ...
              <TableBody>
                <AnimatePresence>
                  {payRunItems.map((item) => {
                    ...
                    const currency = item.foreign_currency || payGroup.currency;
                    const rate = (edits.foreign_base_rate ?? item.foreign_base_rate ?? 0) || 0;
                    const days = (edits.days_worked ?? item.days_worked ?? 0) || 0;
                    const allowances = (edits.foreign_allowances ?? item.foreign_allowances ?? 0) || 0;
                    const exchange = item.exchange_rate || exchangeRate || 1;
                      ...
                      >
                        ...
                        <TableCell>
                          <Input
                            type="text"
                            inputMode="numeric"
                            value={edits.foreign_base_rate !== undefined ? edits.foreign_base_rate.toString() : (item.foreign_base_rate || '').toString()}
                            onChange={(e) => {
                        ...
                        <TableCell>
                          <div className="font-medium text-gray-900 dark:text-foreground">
                            {MultiTenantPayrollService.formatCurrency(baseEarningsFX, currency)}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <Input
                            type="text"
                            inputMode="numeric"
                            value={edits.foreign_allowances !== undefined ? edits.foreign_allowances.toString() : (item.foreign_allowances || '').toString()}
                            onChange={(e) => {
                              const sanitized = e.target.value.replace(/[^\d.]/g, '');
                              handleFieldChange(
                                item.employee_id,
                        ...
                        <TableCell>
                          <div className="font-semibold text-blue-600">
                            {MultiTenantPayrollService.formatCurrency(totalPayFX, currency)}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="font-semibold text-green-600">
                            {MultiTenantPayrollService.formatCurrency(totalPayLocal, 'UGX')}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="font-semibold text-purple-600">
                            {MultiTenantPayrollService.formatCurrency(grossLocal, 'UGX')}
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="font-semibold text-orange-600">
                            {MultiTenantPayrollService.formatCurrency(grossFX, currency)}
                          </div>
                        </TableCell>
...
export default MultiTenantExpatriatePayRunDetails;
