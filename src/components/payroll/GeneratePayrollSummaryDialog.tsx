...
export const GeneratePayrollSummaryDialog = ({ open, onOpenChange, payRunId }: GeneratePayrollSummaryDialogProps) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      // Also load context if not loaded, to enable permission check immediately? 
      // Actually we load it on generate usually, but for permission check we might need it earlier.
      // Or we can just let it be false until loaded.
      // But button visibility depends on it. Ideally we fetch context when dialog opens.
      if (!payRunContext) loadPayRunContext();
    }
  }, [open]);
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto modern-dialog">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              title={!canExport ? "You do not have permission to export payroll reports from this scope" : ""}
            >
              {generating ? (
                <>
